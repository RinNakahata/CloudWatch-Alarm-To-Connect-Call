...
# 使用するS3バケット名と、最新ログ情報のキー
BUCKET_NAME = 'amazon-connect-list'
LOG_OUTPUT_KEY_RECORD = 'connect-call-log/latest_log_key.json'
...
def lambda_handler(event, context):
    try:
        ...
        # ContactId を含むログのみ抽出
        logs = [json.loads(e['message'])
                for e in log_events if 'ContactId' in e['message']]
        ...
        # 最新のログファイル情報（どのExcelファイルを編集すべきか）をS3から取得
        log_info_obj = s3.get_object(
            Bucket=BUCKET_NAME, Key=LOG_OUTPUT_KEY_RECORD)
        log_info = json.loads(log_info_obj['Body'].read().decode('utf-8'))
        log_key = log_info["log_key"]  # 対象のExcelファイルのS3キー
        contact_index = log_info.get("contact_index", 0)
        ...
        # ContactId に一致する行を探す
        matched_row = None
        for row in ws.iter_rows(min_row=2):  # データ行のみループ
